...
def DistFunc(X, Y):
    return -(tf.matmul(X, Y)*2 - tf.reduce_sum(tf.square(Y), 0) - tf.reduce_sum(tf.square(X), 1, keep_dims=True))
...
with graph.as_default():
    ...
    optimizer = tf.train.AdamOptimizer(LR, beta1=0.9, beta2=0.99, epsilon=1e-5).minimize(-logProb)
...
with tf.Session(graph=graph) as session:
    ...
    for i in range(MAX_ITER):
        _, output1, output2, mu, logVar, logPi = session.run([optimizer, posterior, logProb, weights, bias_sigma, bias_pi], feed_dict={inputPL:data})
        if (i % 50) == 0:
            import pylab as plt
            plt.figure()
